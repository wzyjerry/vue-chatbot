...
export default function(source) {
  ...
  // 绘制初始文字
  nodeEnter.append("text").style("fill-opacity", 0);
  // 绘制交互圈
  nodeEnter
    .filter(function(d) {
  ...
  // Update
  let nodeUpdate = nodeEnter.merge(node).attr("class", function(d) {
  ...
  nodeUpdate
    ...
    .text(function(d) {
      ...
      return `${d.data.name ||
        d.data.intent ||
        d.data.type}${flag && d.data.text ? `->${d.data.text}` : ""}`;
    })
    .attr("x", function(d) {
  ...
  const tipLink = svgGroup.selectAll("path.tipLink").data(states.route, 0);
  const tipLinkEnter = tipLink
    .enter()
    .insert("path", "g")
    .attr("class", "tipLink")
    .attr("d", function(d) {
      ...
      return `M${
        d.source.y
      },${d.source.x}A${dr},${dr} 0 0,1 ${d.source.y},${d.source.x}`;
    });
  tipLinkEnter
    .merge(tipLink)
    .transition()
    ...
    .attr("d", function(d) {
      ...
      return `M${
        d.source.y
      },${d.source.x}A${dr},${dr} 0 0,1 ${d.target.y},${d.target.x}`;
    })
    .transition()
